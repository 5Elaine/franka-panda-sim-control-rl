...
class CartesianApproach(Node):
    def __init__(self) -> None:
        super().__init__("cartesian_micro_transport_y")
    ...
    def compute_path(self):
        ...
        waypoint.position.y = start.y + 0.050
        waypoint.position.z = start.z
        ...
        self.get_logger().info(
            "Computing collision-aware 5 cm lateral transport: "
            f"y={start.y:.3f} -> {waypoint.position.y:.3f} m, "
            f"z={waypoint.position.z:.3f} m"
        )
